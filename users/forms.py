...
class FournisseurRegister(UserCreationForm):
    # Create phone field in fournisseur form
    phone = PhoneNumberField(
        ...
        region='MG',  # This sets Madagascar as the default region
        error_messages={
    ...
    class Meta:
        model = User
        # Puts the custom phone number field in the fields list
    ...
    def clean_number(self):
        ...
        if User.objects.filter(phone = number).exists():
            raise forms.ValidationError('Le numéro de téléphone est déja associé à un Tsena')
        return number
